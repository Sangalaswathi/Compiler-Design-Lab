...
def parsing_program(table,term,nonterm,input):
	print("{:<10}".format('Stack'),end=' ')
	print("{:^20}".format('Input string'),end='\t')
	print("Action")
	input = input+'$'
	cols = term+['$']+nonterm
	input = list(input)
	...
	stack = [0]
	while True:
		print("{:<10}".format(''.join(list(map(str,stack)))),end=' ')
		print("{:^20}".format(''.join(input[ip:])),end='\t')
		sp = stack[-1]
		op = table[sp][cols.index(input[ip])]
		if op=='':
			print("error")
			return False
		if op=='Accept':
			...
			print(op)
			stack.append(input[ip])
			stack.append(int(op[1:]))
			ip +=1
		elif op[0] == 'R':
			print(op)
			rule = rules[int(op[1:])-1].split("->")
			lhs = rule[0].strip()
			rhs = rule[1].strip()
			for i in range(len(rhs)-rhs.count(' ')):
				stack.pop()
				stack.pop()
			sp = stack[-1]
			stack.append(lhs)
			stack.append(int(table[sp][cols.index(lhs)]))
...
rules = ['S -> C C','C -> c C','C -> d']
input_string = input("Enter input string : ")
accepted = parsing_program(table,term_userdef,nonterm_userdef,input_string)